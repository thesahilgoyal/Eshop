...
class PaidMembershipForm extends FormHandler implements IFormHandler
{
    ...
    {
        update_user_meta($user_id,'mo_phone_number',$_POST['phone_paidmembership']);
    }
    ...
    private function startOTPVerificationProcess($data)
    {
        if(strcasecmp($this->_otpType, $this->_typePhoneTag)==0){
            $this->sendChallenge('','',null, trim($data['phone_paidmembership']),"phone");
        } elseif(strcasecmp($this->_otpType, $this->_typeEmailTag)==0){
            $this->sendChallenge('',$data['bemail'],null,$data['bemail'],"email");
        }
    }
