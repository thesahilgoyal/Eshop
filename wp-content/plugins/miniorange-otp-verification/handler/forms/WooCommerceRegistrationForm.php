...
class WooCommerceRegistrationForm extends FormHandler implements IFormHandler
{
    ...
    private function checkIntegrityAndValidateOTP($data,WP_Error $errors)
    {
        ...
        $otpVerType = $this->getVerificationType();
        $this->validateChallenge($otpVerType,NULL,$data['moverify']);
    ...
    private function checkIntegrity($data,WP_Error $errors)
    {
        if(strcasecmp($this->_otpType,$this->_typePhoneTag)==0) {
            if(!Sessionutils::isPhoneVerifiedMatch($this->_formSessionVar,$data['billing_phone'])) {
                return new WP_Error(
            ...
            if(!SessionUtils::isEmailVerifiedMatch($this->_formSessionVar,$data['email'])) {
                return new WP_Error(
